...
class _AddGoatScreenState extends State<AddGoatScreen> {
  ...
  Future<bool> _isGoatTagExists(String tagId) async {
    final goats = await _databaseHelper.getGoats();
    return goats.any((goat) => goat['goatTag'] == tagId);
  }
                    ...
                    children: [
                      ...
                      InkWell(
                        ...
                        child: InputDecorator(
                          decoration: InputDecoration(
                            labelText: 'Date of Birth or Purchase Date',
                          ...
                          ElevatedButton.icon(
                            ...
                            onPressed: () async {
                              if (_formKey.currentState!.validate()) {
                                bool tagExists = await _isGoatTagExists(
                                ...
                                if (tagExists) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                        'Goat with that tag ID is already saved. Please enter a different tag ID.',
                                      ),
                                ...
                                int result = await _databaseHelper.insertGoat(
                                  goat,
                                );
                                ...
                                if (result > 0) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                        'Failed to save goat. Please try again.',
                                      ),
                                      backgroundColor: Colors.red,
                                    ),
                                  );
